...
const int num_ball = 40;
bool space = false;
bool pause = false;
double d_x, d_z;
// There are four balls
// initialize the position (coordinate) of each ball (ball0 ~ ball3)
const float spherePos[num_ball][2] = { {-2.9f,1.7},{-2.4f,1.7},{-1.9f,1.7}, {-3.3f,1.3},{-3.3f,0.7},{-3.3f,0.2},{-3.3f,-0.4}, {-1.5f,1.3},{-1.5f,0.7},{-1.5f,0.2},{-1.5f,-0.3}, {-2.9f,-0.7},{-2.4f,-0.7},{-1.9f,-0.7},
									   ...
									   {2.03f,1.7},{2.48f,1.7},{2.93f,1.7}, {1.61f,1.3},{1.61f,0.7},{1.61f,0.2},{1.61f,-0.3},{1.61f,-0.8}, {3.3f,1.3} ,{2.2f,0.7},{2.8f,0.7}, {2.9f,0.7} };
// initialize the color of each ball (ball0 ~ ball3)
const D3DXCOLOR sphereColor[num_ball] = {};
...
public:
	...
	bool hasIntersected(CSphere& ball)
	{
		...
		float diff_x = this->getCenter().x - ball.getCenter().x;
		float diff_z = this->getCenter().z - ball.getCenter().z;
		float distanceSquared = diff_x * diff_x + diff_z * diff_z;

		// Calculate the sum of the radii squared
		float radiiSumSquared = (this->getRadius() + ball.getRadius()) * (this->getRadius() + ball.getRadius());
	...
	void hitBy(CSphere& ball)
	{
		...
		if (hasIntersected(ball)) {
			// Calculate the vector from the center of the current sphere to the center of the other sphere
			float delta_x = this->getCenter().x - ball.getCenter().x;
			float delta_z = this->getCenter().z - ball.getCenter().z;

			// Calculate the speed and time of impact
			...
			// Update the velocity of the other sphere
			ball.setPower(-delta_x / delta_time, -delta_z / delta_time);

			// Move the current sphere to a non-visible location if it's not the control ball
			if (!control_ball) {
	...
	bool getControlBall() { return this->control_ball; }
	...
	void setControlBall(bool control_ball)
	{
...
public:
	...
	bool hasIntersected(CSphere& ball) {
		// Check if the ball is within the boundaries of the wall
		...
		return (ball.getCenter().x >= l_limit && ball.getCenter().x <= r_limit && ball.getCenter().z >= t_limit);
	}
		
	void hitBy(CSphere& ball) {
		// Check for intersection
		if (hasIntersected(ball)) {
			// Calculate the limits of the wall
			float l_limit = this->m_x - this->m_width / 2 - M_RADIUS;
			float r_limit = this->m_x + this->m_width / 2 + M_RADIUS;
			float t_limit = this->m_z - this->m_depth / 2 - M_RADIUS;

			// Bounce the ball back if it hits the top side of the wall
			if (ball.getCenter().z == t_limit) {
				ball.setPower(ball.getVelocity_X(), -ball.getVelocity_Z());
			}
			// Bounce the ball back if it hits the left side of the wall
			if (ball.getCenter().x == l_limit) {
				ball.setPower(-ball.getVelocity_X(), ball.getVelocity_Z());
			}
			// Bounce the ball back if it hits the right side of the wall
...
CWall	g_legoPlane;
CWall	g_legowall[3];
CSphere	g_sphere[num_ball];
CSphere	g_ControlBall;
CSphere g_BulletBall;
CLight	g_light;
// Add a global variable to keep track of remaining lives
int remainingLifes = 5;
...
//			over = true;
            ...
            remainingLifes--;
            if (remainingLifes <= 0) {
                // No remaining lives, disable control but keep the game running
                over = true;
                space = false;
                g_ControlBall.setPower(0, 0);
                remainingLifes = 0; // Ensure remainingLives is not negative
            }
        }
...
void HandleInput(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN:
        ...
        case 0x52: // R key
            if (over) {
                over = false;
                space = false;
                remainingLifes = 5; // Reset lives for the next game
                ...
                // Respawn control and bullet ball
                g_ControlBall.setCenter(0.0f, (float)M_RADIUS, -3.0f);
                RespawnBulletBall();
            }
            break;
        ...
        case 0x50:
            if (!over) {
                pause = !pause;
                if (pause) {
                    d_x = g_BulletBall.getVelocity_X();
                    d_z = g_BulletBall.getVelocity_Z();
                    g_BulletBall.setPower(0, 0);
                }
                else {
                    g_BulletBall.setPower(d_x, d_z);
                }
            }
            break;
        default:
    ...
    case WM_MOUSEMOVE:
        ...
        if (!pause && !over && normalizedX > l_limit && normalizedX < r_limit) {
