...
export default function WritePinStory({ isOpen, closeModal, addPin }) {
  ...
  const handlePost = () => {
    //addPin({ cotitle, type, content, imageFiles, shortVideo, platforms, profilePublic, platformLinks, tags, monetize });
    closeModal();
  };
          ...
          <div className="px-4 md:px-6 space-y-4 overflow-x-hidden">
            <div className="grid grid-cols-2 gap-4 h-64">
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 relative overflow-hidden">
                {state.imageFiles.length > 0 ? (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <img src={URL.createObjectURL(imageFiles[0])} alt="Uploaded" className="w-full h-full object-cover" />
                  </div>
                ) : (
                  <>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 relative overflow-hidden">
                {state.shortVideo ? (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <video src={URL.createObjectURL(shortVideo)} className="w-full h-full object-cover" />
                  </div>
                ) : (
                  <>
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="flex items-center">
              ...
              <button onClick={() => handleMonetizeChange(true)} className={`px-2 py-1 rounded ${state.monetize === true ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700'}`}>예</button>
              <button onClick={() => handleMonetizeChange(false)} className={`px-2 py-1 ml-2 rounded ${state.monetize === false ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700'}`}>아니오</button>
            </div>
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 modal-overlay" onClick={handleOutsideClick}>
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-6 mx-4 md:mx-0 md:max-w-2xl z-10 overflow-none" style={{ height: '80vh', maxHeight: '800px' }}>
